...
template <typename T>
using pod_type_tracing = std::integral_constant<bool, std::is_trivially_copyable_v<T>>;
...
template <typename T>
void copy_array(T* ASH_RESTRICT dst, T const* ASH_RESTRICT src, size_t size, pod_type<T>) {
    memcpy(dst, src, sizeof(T) * size);
}

template <typename T>
void copy_array(T* ASH_RESTRICT dst, T const* ASH_RESTRICT src, size_t size, non_pod_t<T>) {
...
template <typename T>
void copy_array(T* ASH_RESTRICT dst, T const* ASH_RESTRICT src, size_t size) {
    using namespace _arrayctl;
    copy_array(dst, src, size, pod_type_tracing<T>());
}

template <typename T>
void move_array(T* ASH_RESTRICT dst, T const* ASH_RESTRICT src, size_t size) {
    using namespace _arrayctl;
